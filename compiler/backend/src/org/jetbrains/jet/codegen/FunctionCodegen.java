...
public class FunctionCodegen extends GenerationStateAware {
    ...
    private void generateParameterAnnotations(
        ...
        for (int i = 0; i < kotlinParameterTypes.size(); i++) {
            JvmMethodParameterSignature paramSignature = kotlinParameterTypes.get(i);
            JvmMethodParameterKind kind = paramSignature.getKind();
            ...
            if (kind == JvmMethodParameterKind.ENUM_NAME || kind == JvmMethodParameterKind.ENUM_ORDINAL) {
                generateAnnotationForEnumConstructorParameter(mv, i);
            }
        }
    }
    ...
    private static void generateAnnotationForEnumConstructorParameter(MethodVisitor mv, int i) {
    ...
    private void generateMethodBody(
        ...
        generateLocalVariableTable(mv, signature, functionDescriptor, thisType, methodBegin, methodEnd, kind);
    }
